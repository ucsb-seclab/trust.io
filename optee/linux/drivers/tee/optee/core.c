...
struct tee_device *trustio_glob_dev;
struct tee_context *trustio_glob_ctx;
int is_trustio_glob_init = 0;

long trustio_total_times = 0;

EXPORT_SYMBOL_GPL(trustio_total_times);

ktime_t trustio_sec_prot_start;
unsigned long trustio_sec_prot_ms_time =  0;

EXPORT_SYMBOL_GPL(trustio_sec_prot_start);
EXPORT_SYMBOL_GPL(trustio_sec_prot_ms_time);

ktime_t trustio_ns_callback_start;
unsigned long trustio_ns_cb_ms_time = 0;

EXPORT_SYMBOL_GPL(trustio_ns_callback_start);
EXPORT_SYMBOL_GPL(trustio_ns_cb_ms_time);

ktime_t trustio_sec_verify_start;
unsigned long trustio_sec_verify_ms_time = 0;

EXPORT_SYMBOL_GPL(trustio_sec_verify_start);
EXPORT_SYMBOL_GPL(trustio_sec_verify_ms_time);
/**
...
static int optee_open(struct tee_context *ctx)
{
	...
	ctx->data = ctxdata;
	if(!is_trustio_glob_init) {
	    is_trustio_glob_init = 1;
    	optee_open(trustio_glob_ctx);
	}
	return 0;
}
...
static int optee_probe(struct platform_device *pdev)
{
	...
	trustio_glob_dev = teedev;
	trustio_glob_ctx = kzalloc(sizeof(*trustio_glob_ctx), GFP_KERNEL);
	
	trustio_glob_ctx->teedev = teedev;
	INIT_LIST_HEAD(&trustio_glob_ctx->list_shm);
...
EXPORT_SYMBOL_GPL(trustio_glob_dev);
