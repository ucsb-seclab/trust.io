				...
				struct tee_shm *shm)
{
	...
	default:
	    tmpend = ktime_get();
	    actual_time = (unsigned long)ktime_to_ns(ktime_sub(tmpend, trustio_sec_prot_start));
	    trustio_sec_prot_ms_time += actual_time;
	    
		handle_rpc_supp_cmd(ctx, arg);
		trustio_ns_callback_start = ktime_get();
		actual_time = (unsigned long)ktime_to_ns(ktime_sub(trustio_ns_callback_start, tmpend));
		trustio_ns_cb_ms_time += actual_time;
