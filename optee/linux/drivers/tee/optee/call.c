...
extern struct tee_device *trustio_glob_dev;
extern struct tee_context *trustio_glob_ctx;

//TRUST_IO_CALL
u32 trustio_tz_call(long target_val)
{
	struct optee *optee = tee_get_drvdata(trustio_glob_dev);
	struct optee_call_waiter w;
	struct optee_rpc_param param = { };
	struct tee_shm *shm;
	u32 ret;
	...
	shm = get_msg_arg(trustio_glob_ctx, 2, &msg_arg, &msg_parg);
	if (IS_ERR(shm))
		return PTR_ERR(shm);

	msg_arg->cmd = OPTEE_MSG_CMD_TRUSTIO_GPIO;
	msg_arg->cancel_id = 0;
	msg_param = OPTEE_MSG_GET_PARAMS(msg_arg);
	
	msg_param[0].u.value.a = target_val;
	

	param.a0 = OPTEE_SMC_CALL_WITH_ARG;
	reg_pair_from_64(&param.a1, &param.a2, msg_parg);
	/* Initialize waiter */
	optee_cq_wait_init(&optee->call_queue, &w);
	while (true) {
		struct arm_smccc_res res;
		...
		if (res.a0 == OPTEE_SMC_RETURN_ETHREAD_LIMIT) {
			...
			param.a2 = res.a2;
			param.a3 = res.a3;
			optee_handle_rpc(trustio_glob_ctx, &param);
		} else {
			ret = res.a0;
			break;
		}
...
EXPORT_SYMBOL_GPL(trustio_tz_call);
...
long trustio_tz_read_call(void)
{
	...
	msg_arg->cmd = OPTEE_MSG_CMD_TRUSTIO_GPIO_READ;
	msg_arg->cancel_id = 0;
	msg_param = OPTEE_MSG_GET_PARAMS(msg_arg);
	...
	optee_cq_wait_final(&optee->call_queue, &w);
    tee_shm_free(shm);
	return (long)param.a5;
}

EXPORT_SYMBOL_GPL(trustio_tz_read_call);
